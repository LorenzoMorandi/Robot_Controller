...
supervisor_multi_cross::supervisor_multi_cross():pnh("~") //Constructor
{
    n=0;
}

supervisor_multi_cross::~supervisor_multi_cross() //Desctructor
{
    
}

void supervisor_multi_cross::ReadPoses() //Read from tf the robots position
{
    for(int i = 0; i < n; i++)
    {
...
double supervisor_multi_cross::LinearErrY(geometry_msgs::Pose2D current, std::vector<geometry_msgs::Pose2D> reference)
{
    geometry_msgs::Pose2D reff = reference[0];
    return  reff.y - current.y;
}

double supervisor_multi_cross::LinearErrX(geometry_msgs::Pose2D current, std::vector<geometry_msgs::Pose2D> reference)
{
    geometry_msgs::Pose2D reff = reference[0];
    return  reff.x - current.x;
}

bool supervisor_multi_cross::evolve_state_machines(int i)	//State Machine evolution
{
...
int supervisor_multi_cross::random_generator()
{
...
void supervisor_multi_cross::init()
{
    ...
    try 
    {
	digraphReader(g, ros::package::getPath("robot_controller") + "/graph/prova_grafo_multi.lgf").
	arcMap("length", len).
    ...
    {
	coord_x[n]=coord_x[n]*0.1143;
	coord_y[n]=coord_y[n]*0.1143;
    ...
    Node n4=g.nodeFromId(random_generator());  
    Node n5=g.nodeFromId(random_generator());  
    Node n6=g.nodeFromId(random_generator());  
    Node n7=g.nodeFromId(random_generator());  
    Node n8=g.nodeFromId(random_generator());  
    Node n9=g.nodeFromId(random_generator());  
    Node n10=g.nodeFromId(random_generator());  
    Node n11=g.nodeFromId(random_generator());  
    Node n12=g.nodeFromId(random_generator());  
    Node n13=g.nodeFromId(random_generator());  
    Node n14=g.nodeFromId(random_generator());  
    ...
    goals.push_back(g.nodeFromId(89));
    goals.push_back(g.nodeFromId(88));
    goals.push_back(g.nodeFromId(86));
    goals.push_back(g.nodeFromId(84));
    goals.push_back(g.nodeFromId(82));
    goals.push_back(g.nodeFromId(97));
    goals.push_back(g.nodeFromId(99));
    goals.push_back(g.nodeFromId(101));
    goals.push_back(g.nodeFromId(104));
    goals.push_back(g.nodeFromId(96));
    goals.push_back(g.nodeFromId(94));
    goals.push_back(g.nodeFromId(92));
    goals.push_back(g.nodeFromId(89));
    goals.push_back(g.nodeFromId(92));
    goals.push_back(g.nodeFromId(97));
    goals.push_back(g.nodeFromId(104));
	...
	dijkstra_test.run(random_start_node.at(i), goals.at(i));
	
	std::cout << "PATH Robot" << i << ": ";
	   
	if (dijkstra_test.dist(goals.at(i)) > 0)
	{   
	    for (Node v = goals.at(i); v != random_start_node.at(i); v = dijkstra_test.predNode(v)) 
	    {
...
void supervisor_multi_cross::run()
{
...
/*			if(robots[i].curr_pose.x <= 14.5 && robots[i].curr_pose.x >= 13.5 &&  robots[i].curr_pose.y <= 17.5 && robots[i].curr_pose.y >= 16.5 || 
	...
	for(int i = 0; i < n; i++)	
	{
	    ...
	    {
		robots[i].twist.angular.z = 3*sin(robots[i].err_ang);
		robots[i].twist.linear.x = 0.5*robots[i].err_lin; // MULTI CROSS
	    ...
	    { 
		robots[i].twist.angular.z = 3*sin(robots[i].err_ang);
		robots[i].twist.linear.x = 0.2*robots[i].err_lin; // MULTI CROSS
		if (robots[i].twist.linear.x > 5)
		    robots[i].twist.linear.x = 5;
